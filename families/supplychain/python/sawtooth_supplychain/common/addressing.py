...
class Addressing(object):
    ...
    @classmethod
    def application_address(cls, agent_key, record_identifier):
        address = hashlib.sha256(agent_key.encode()).hexdigest() +\
            hashlib.sha256(record_identifier.encode()).hexdigest()
        return cls.application_namespace() + address

    @classmethod
    def record_namespace(cls):
        if cls._record_namespace is None:
            cls._record_namespace = hashlib.sha512(
    ...
    def record_address(cls, identifier):
        address = hashlib.sha512(identifier.encode()).hexdigest()
