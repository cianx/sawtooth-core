...
class BlockStore(MutableMapping):
    ...
    def _get(self, key, assert_chain_head_id):
        if assert_chain_head_id is not None:
            ...
            current_head_id = result[0][1]
            if current_head_id != assert_chain_head_id:
                raise ChainHeadUpdatedError(
                    "Chain head is now {}, asserted {}".format(current_head_id[:8],
    ...
    def _has(self, key, assert_chain_head_id):
        try:
            self._get(key, assert_chain_head_id)
            return True
        except KeyError:
            return False

    def _get_block(self, key, assert_chain_head_id=None):
        value = self._get(key, assert_chain_head_id)
    ...
    def get_block_by_transaction_id(self, txn_id, assert_chain_head_id=None):
        try:
            ...
            return self._get_block(block_id, assert_chain_head_id)
        except KeyError:
            raise KeyError('Transaction "%s" not in BlockStore', txn_id[:8])

    def has_transaction(self, txn_id, assert_chain_head_id=None):
        return self._has(txn_id, assert_chain_head_id)
    ...
    def get_block_by_batch_id(self, batch_id, assert_chain_head_id=None):
        try:
            ...
            raise KeyError('Batch "%s" not in BlockStore', batch_id[:8])
